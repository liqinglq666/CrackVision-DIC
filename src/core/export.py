...
def prepare_frame_summary(rows: Iterable[dict]) -> pd.DataFrame:
    """Build the frame table while preserving failed measurements as NaN."""
...
def prepare_crack_details(tables: Iterable[pd.DataFrame]) -> pd.DataFrame:
    ...
    for col in ("W_median_mm", "W_avg_mm", "W_95_mm", "W_max_mm", "Slip_median_mm"):
...
def build_qa(frame_df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows: list[dict[str, object]] = [
        ...
        {"Metric": "frames_cod_ok", "Value": int((frame_df["cod_status"] == "ok").sum())},
        {"Metric": "frames_cod_failed", "Value": int((frame_df["cod_status"] != "ok").sum())},
        {"Metric": "median_valid_fraction", "Value": float(pd.to_numeric(frame_df["valid_fraction"], errors="coerce").median())},
    ]
    ...
    for col in ("pixel_size_mm", "dic_step_px", "dic_point_spacing_mm", "metadata_source"):
...
def export_workbook(path: Path, frame_df: pd.DataFrame, crack_df: pd.DataFrame, qa_df: pd.DataFrame) -> None:
    path = Path(path)
    ...
    with pd.ExcelWriter(path, engine="openpyxl") as writer:
        ...
        for ws in writer.book.worksheets:
            ...
            for idx, column in enumerate(ws.columns, start=1):
                max_len = min(60, max((len(str(cell.value)) if cell.value is not None else 0 for cell in column), default=0) + 2)
