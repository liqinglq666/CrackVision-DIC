...
def _fast_cod_rigorous_kernel(
    ...
    start_d = max(1, int(delta_px))
    end_d = start_d + max(1, max_search_px)
    ...
    for i in range(n):
        ...
        if has_v:
            if np.isnan(v_p) or np.isnan(v_n):
                continue
            dv = v_p - v_n
            widths[cnt] = abs(du * nx + dv * ny) * ratio
        else:
            widths[cnt] = abs(du * nx) * ratio
        valid_idx[cnt] = i
        cnt += 1
...
class CrackPhysicsEngine:
    def __init__(self, config: dict):
        ...
        self.delta = float(sampling.get("delta_px", 3.0))
        self.max_search_px = int(sampling.get("max_search_px", sampling.get("Delta_px", 15)))
        self.closing_radius = int(phys.get("morphology_closing_radius", 0))
    ...
    def extract_skeleton(self, exx: np.ndarray, mask: np.ndarray) -> np.ndarray:
        valid = mask & np.isfinite(exx)
        clean_exx = exx[valid]
        if clean_exx.size == 0:
            return np.zeros_like(exx, dtype=bool)

        median = float(np.median(clean_exx))
        mad = float(np.median(np.abs(clean_exx - median)))
        robust_sigma = mad * 1.4826
        threshold = min(max(median + self.k * robust_sigma, self.min_s), 0.005)

        internal_nans = np.zeros_like(mask, dtype=bool)
        if self.closing_radius > 0:
            closed_mask = morphology.closing(mask, morphology.disk(self.closing_radius))
    ...
    def compute_cod(
        ...
        if has_v:
            v_data = np.ascontiguousarray(v_map)
        else:
            v_data = np.zeros_like(u_map)

        widths, valid_idx = _fast_cod_rigorous_kernel(
        ...
        summary["L_mm"] = summary["count"] * ratio
        summary = summary[
        ...
        return {
            ...
            "raw_widths": raw,
            "per_crack_details": summary.rename(
                columns={"mean": "W_avg_mm", "max": "W_max_mm", "L_mm": "Length_mm"}
            ),
        }
